...
defineProps<{
  toolMode: boolean;
  selectedToolCards: DrawnCard[];
}>();

const emit = defineEmits<{
  (e: "open-fortune"): void;
  (e: "open-tools"): void;
  (e: "clear-tool-cards"): void;
}>();
...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- 已加入的卡牌 -->
    <div
      v-if="selectedToolCards.length > 0"
    ...
    >
      <div class="max-w-3xl mx-auto flex items-center gap-2 flex-wrap">
        ...
        <div
          v-for="(drawnCard, index) in selectedToolCards"
          :key="index"
        ...
        <button class="btn btn-ghost btn-xs" @click="emit('clear-tool-cards')">
          清除
        </button>
      </div>
    </div>
